...
class Palace(CMakePackage, CudaPackage, ROCmPackage):
    ...
    def cmake_args(self):
        ...
        if "+cuda" in self.spec and "none" not in self.spec.variants["cuda_arch"].value:
            args += [
                self.define(
        ...
        if (
            "+rocm" in self.spec
            and "none" not in self.spec.variants["amdgpu_target"].value
        ):
            args += [
                self.define(
        ...
        args += [
            ...
            self.define(
                "BLAS_LIBRARIES",
                "{0}".format(self.spec["blas"].libs.joined(";")),  # type: ignore
            ),
            self.define(
                "LAPACK_LIBRARIES",
                "{0}".format(self.spec["lapack"].libs.joined(";")),  # type: ignore
            ),
        ]
        ...
        if "+libxsmm" in self.spec:
            args += [self.define("LIBXSMM_DIR", self.spec["libxsmm"].prefix)]  # type: ignore
        if "+magma" in self.spec:
            args += [self.define("MAGMA_DIR", self.spec["magma"].prefix)]  # type: ignore
