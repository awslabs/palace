...
@testset "inline_ref merges keys" begin
    ...
    raw = root["\$defs"]["PEC"]["properties"]["Attributes"]
...
@testset "render_schema_section! produces valid markdown" begin
    ...
    @test contains(md, "config-keypath")
    # All leaf fields in one <dl> block per section level
    @test count("<dl class=\"palace-config\">", md) == 2
end
