...
function type_string(root::AbstractDict, schema::AbstractDict)::String
    ...
    if haskey(schema, "oneOf")
        # Resolve \$ref branches first: main's schemas hoist variants into \$defs.
        branches = [inline_ref(root, b) for b in schema["oneOf"]]
...
Branches are resolved against root first (\$ref variants hoisted into \$defs).
"""
function is_object_dispatch(root::AbstractDict, branches::Vector)::Bool
    isempty(branches) && return false
    resolved = [inline_ref(root, b) for b in branches]
    all(haskey(b, "const") for b in resolved) && return false
    all(!haskey(b, "properties") && !haskey(b, "type") for b in resolved) && return false
    return any(haskey(b, "properties") for b in resolved)
end

"""
Merge undiscriminated object variants (e.g. LumpedPort's "Attributes"-vs-"Elements"
alternatives) into a single object schema for documentation: union of properties
(first occurrence wins, preserving JSON order), intersection of required (only
fields required by every variant are badged required — the alternatives themselves
are described in their field descriptions).
"""
function merge_variant_schemas(root::AbstractDict, variants::Vector)::AbstractDict
    props = OrderedDict{String, Any}()
    req = nothing
    desc = ""
    title = ""
    for v in variants
        rv = inline_ref(root, v)
        haskey(rv, "properties") || continue
        for (k, p) in rv["properties"]
            haskey(props, k) || (props[k] = p)
        end
        r = Set{String}(string.(get(rv, "required", String[])))
        req = req === nothing ? r : intersect(req, r)
        isempty(desc) && (desc = get(rv, "description", ""))
        isempty(title) && (title = get(rv, "title", ""))
    end
    out = OrderedDict{String, Any}(
        "type" => "object",
        "required" => sort!(collect(something(req, Set{String}()))),
        "properties" => props
    )
...
function collect_fields(
    ...
    for (name, raw_field) in props
        ...
        elseif t == "array" && haskey(field, "items")
            ...
            elseif haskey(items, "oneOf") && is_object_dispatch(root, items["oneOf"])
                fis_array = true
                resolved_variants = [inline_ref(root, v) for v in items["oneOf"]]
                filter!(rv -> haskey(rv, "properties"), resolved_variants)
                if all(rv -> extract_discriminator_key(rv, "") == "", resolved_variants)
                    # Undiscriminated alternatives (required "Attributes" OR "Elements"):
                    # document as one merged item schema rather than near-identical
                    # variant sections.
                    push!(sub_schemas, "" => merge_variant_schemas(root, resolved_variants))
                else
        ...
        elseif !is_const_enum &&
               haskey(field, "oneOf") &&
               is_object_dispatch(root, field["oneOf"])
...
function render_field_entry!(buf::IOBuffer, f::FieldDoc)
    ...
    print(
        buf,
        " <span class=\"tag config-type\">type: <code>$(html_escape(f.type_str))</code></span>"
    )
    if f.required
