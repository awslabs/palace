...
excitations carry unit instantaneous power at unit pulse amplitude. Wave port modes are
normalized by the magnitude of their complex power, ``\vert\int (\bm{E}\times\bm{H}^*)\cdot\bm{n}\,dS\vert``,
which equals the transported power only for a lossless port cross section (see issue #960
for the lossy case).
