...
[This script](scripts/validate_config) uses
Julia's [`JSONSchema.jl`](https://github.com/fredo-dedup/JSONSchema.jl) and the Schema
provided in [`scripts/schema/`](scripts/schema)
...
Below is a sample flamegraph resulted from profiling the rings example located at [`/examples/rings/rings.json`](examples/rings/rings.json) with polynomial order P=6 (`Solver["Order"]: 6`).
