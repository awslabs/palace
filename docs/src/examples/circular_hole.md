...
Whether a film screens perfectly or penetrates is set by how its attribute is declared:
  ...
  - **Perfect conductor (``\lambda \to 0``).** A film attribute listed under `"FilmAttributes"`
...
generated by `mesh/square_hole_mesh.jl`). The film attribute is listed under `"FilmAttributes"` but
...
The fields have the following meaning:
  ...
  - `"FilmAttributes"`: boundary attributes of the metal film carrying the flux loop.
...
separated London film, each with a distinct `"FilmAttributes"` attribute (both films appear in the
