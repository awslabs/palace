...
power pattern on the E-plane (xz-plane) is given by

```math
P(\theta) = \left|\frac{\cos\left(\frac{\pi}{2}\cos\theta\right)}{\sin\theta}\right|^2\,,
```
...
!!! note "Far-field considerations"
    
    For accurate far-field analysis, the observation distance must satisfy ``r_{max} / \lambda \gg 1``. However, such meshes quickly become computationally expensive
    since the cost scales as ``r_{max}^3``. In this example, we place the outer
    boundary at ``r_{max} = 2.5\lambda`` (resulting in mesh with approximately 130k
    elements), which is not quite far enough for true far-field extraction. We will
    demonstrate that results improve significantly when using ``r_{max} = 5\lambda``
    (mesh with approximately 680k elements), albeit at increased computational cost
    (requiring over 70 GB of memory).
...
axis and nulls approximately along the antenna axis. However, there is no full
convergence for this simulation, and even at ``r_{max}``, we would not achieve
complete agreement.

This discrepancy with theory is expected for several physical reasons,
including:

  - The antenna is not infinitely thin
  - The absorbing boundary conditions, while effective, are not perfect at
    preventing all reflections from the computational boundary
  - The observation regions are at ``r < 2.5\lambda`` are not yet firmly in the
    far-field zone

To demonstrate the importance of far-field distance, we can extend the outer
boundary from ``r_{max} = 2.5\lambda`` to ``r_{max} = 5\lambda``. While this
significantly increases computational cost (scaling roughly as the cube of the
domain size), it provides closer agreement with theoretical expectations.
...
The improved results with the larger computational domain confirm that accurate
far-field analysis requires sufficient distance from the radiating structure,
