...
If you are developing palace, or just getting started, you can also consider using our supported `devcontainer`. More information on this is described in [Build using a devcontainer](#Build-using-a-devcontainer).
...
## Build using a devcontainer

[Development Containers](https://containers.dev/) (dev conatiners for short) are an IDE agnostic tool that allow you to use a container + JSON specification for a full-featured development environment.

While you are free to use Docker, Podman, Charliecloud, or your favourite docker compatible container runtime, these docs will outline the process using [`finch`](https://runfinch.com/). This documentation will also assume you are using VSCode, but other IDEs offer similar functionality.

### Initial Configuration

1. Make sure you install the [Dev Container](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers) extension for VSCode (or for your IDE of choice).
1. Based on your OS, configure your settings to support [DevContainers on Finch](https://runfinch.com/docs/integrations/devcontainers-on-finch/).

#### GitHub Container Registry

Since our devcontainer is running the same OS and Spack version as our GitHub actions spack CPU builds, we can leverage the GHCR cache here to speed up builds. Unfortunately this cache is currently limited to people with developer access to the repository, but it might be made public in the future.

Assuming you are a developer and have access to [this cache](https://github.com/awslabs/palace/pkgs/container/palace):

1. Generate a `Tokens (classic)` Personal Access Token (PAT) with `read:packages` permission to package registries (note that I couldn't get fine-grained tokens working).
1. Add that token to a `.secrets` file in your palace root that contains your generated secret in a "key=value" format: `GITHUB_PAT=github_pat_xxx`.
1. Add your GitHub username to the same `.secrets` file in the form `GITHUB_USERNAME=<username>`.



### Running

In VSCode, all you have to do is `⌘+⇧+p` (command + shift + p), and run the command `Dev Containers: Open Folder in Container`.

### Modifying

Since you may want to modify the configuration, the following files are relevant:

- `.devcontainer`
  - `devcontainer.json` is the main configuration file that controls the extensions and customisations on top of the base container being run.
  - `Dockerfile` is automatically generated by the shell script, but can be modified if you want small cusomtizations.
  - `generate_dockerfile.sh` is a script used to generate the Dockerfile. This script assumes you have a spack installation under the environment variable `${SPACK_ROOT}` that works, and that you are running from the palace root directory. Note that some sed commands are finnicky, so we rely on `gnu-sed`. You may need to `brew install gnu-sed` on MacOS and configure your path accordingly.
- `spack/local/packages/palace/package.py` is the local `package.py` that we use to define the palace being built within the container.
