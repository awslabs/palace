...
```@example
run(Cmd(`spack --version`)); nothing # hide
```
...
You should see:
```@example
# We need to eval because spack load --sh returns a list of exports to evaluate in the current shell # hide
run(`sh -c "eval \$(spack load --sh palace) && palace --help"`); nothing # hide
```
...
the mesh. All simulation types in *Palace* have some form of iteration (over
frequencies, times, mode numbers, or terminals). We save the output in the
`postpro` folder, which will be overwritten if it already exists. See the
[problem types documentation](config/problem.md#problem-types) for details on
all available problem types and their outputs.
...
parameter. For example, `L0` of `1e-2` means that one unit on the mesh is one
centimeter. The `Model` section can also include settings for adaptive mesh
...
documentation](config/domains.md#domains["Materials"]).

The `Domains` section also includes a `Postprocessing` subsection for
calculating specific quantities. In this example, we add:
- `Energy` activates integrated energy calculations in the 3D domain
- `Probe` requests field values at specific coordinates defined by a `Center`
  (in mesh units)

When configuring `Postprocessing`, you must specify an `Index` that determines
the suffix for column headers in the output CSV files. For example, with `Index:
1`, the probe output will show headers like `E_x[1]`.
...
a default behavior: any surface without an explicit boundary condition is
treated as a Perfect Magnetic Conductor (PMC), where the normal component of the
...
activates each terminal sequentially, applying a unit of potential and solving
Maxwell's equations. Each of these steps adds a new row to the output CSV files.
...
algorithm parameters in `"Linear"` are not essential for this tutorial.
...
```@example
run(`sh -c "eval \$(spack load --sh palace) && palace --dry-run spheres.json"`); nothing # hide
```
...
```
`-np 1` instruct *Palace* to run on with single MPI process.
...
```
```@example json
run(Cmd(`../../build/bin/palace -np 1 spheres.json`)); nothing # hide
```
```@raw html
</div>
```
...
```@example json
run(Cmd(`tree postpro`)); nothing # hide
```
...
```@example json
run(Cmd(`cat postpro/probe-E.csv`)); nothing # hide
```
...
```@example json
run(Cmd(`cat postpro/terminal-C.csv`)); nothing # hide
```
...
2. Explore the [examples](examples/examples.md) to see different problem types
   and more complex geometries
3. Read the [configuration reference](config/config.md) to understand all
   available options
