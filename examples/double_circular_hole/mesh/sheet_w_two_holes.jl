...
Generate a mesh for a rectangular sheet with two circular holes using Gmsh

# Arguments

  - filename - the filename to use for the generated mesh
  - L_outer - outer computational box size
  - rect_width - width of the rectangular sheet
  - rect_height - height of the rectangular sheet
  - hole_radius - radius of circular holes in the sheet
  - hole_separation - distance between hole centers
...
function generate_sheet_with_two_holes_mesh(;
    ...
    println("Sheet surface: 8")
    println("Left hole surface: 9")
    println("Right hole surface: 10")
    println()
