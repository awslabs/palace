...
Return angles and patterns for theoretical electrical current dipole radiation pattern.
Based on geosci.xyz far-field analytical solution for z-oriented dipole.

For z-oriented dipole, converting from geosci.xyz x-oriented expressions:

  - E-plane (xz): |E_θ|² ∝ |cos(θ)|² (dominates along dipole direction)
  - H-plane (xy): |H_φ|² ∝ constant (omnidirectional)
"""
function generate_theoretical_dipole()
    angles = 0:360

    # E-plane: For z-oriented current dipole
    # Standard dipole pattern: nulls along dipole axis, maxima perpendicular to axis
    ...
    # H-plane: For z-oriented dipole, H_φ is constant (omnidirectional in xy-plane)
