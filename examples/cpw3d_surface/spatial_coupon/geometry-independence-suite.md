...
with ratio 2, extruded along the edge at the tangential spacing, explicit pyramids
...
- prism edge tubes on the top and bottom edge of every straight metal segment
  ...
  bottom edge; the etch footprint must carry the metal edge), extruded in layers of
  spacing <= TangentialSize (the spacing actually used divides the tube length and
  is recorded per tube), lateral quadrangles closed by explicit pyramids of height
  0.5 x the outermost ring size; tubes end at the outer box and R / tan(phi / 2) +
  h_K before a semantic corner (phi the in-plane angle of the metal edges meeting
  there; recorded);
