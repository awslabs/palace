...
function generate_grating_mesh(;
    ...
    # --- Physical groups ---
    vac_vols = [
        volumes[(ix, iy, iz)] for ix = 1:3 for iy = 1:3 for
        iz = 1:3 if !(ix == 2 && iy == 2 && iz == 2)
    ]
    ...
    gmsh.model.addPhysicalGroup(
        2,
        [surfs_x[(1, iy, iz)] for iy = 1:3 for iz = 1:3],
        1,
        "x_min"
    )
    gmsh.model.addPhysicalGroup(
        2,
        [surfs_x[(4, iy, iz)] for iy = 1:3 for iz = 1:3],
        2,
        "x_max"
    )
    gmsh.model.addPhysicalGroup(
        2,
        [surfs_y[(ix, 1, iz)] for ix = 1:3 for iz = 1:3],
        3,
        "y_min"
    )
    gmsh.model.addPhysicalGroup(
        2,
        [surfs_y[(ix, 4, iz)] for ix = 1:3 for iz = 1:3],
        4,
        "y_max"
    )
    gmsh.model.addPhysicalGroup(
        2,
        [surfs_z[(ix, iy, 1)] for ix = 1:3 for iy = 1:3],
        5,
        "port1"
    )
    gmsh.model.addPhysicalGroup(
        2,
        [surfs_z[(ix, iy, 4)] for ix = 1:3 for iy = 1:3],
        6,
        "port2"
    )
    ...
    # Set up Gmsh periodic mesh matching (optional — enables running without explicit
    # Translation vectors in Palace config).
    # x-periodicity: x_max surfaces are periodic copies of x_min
    xmin_surfs = [surfs_x[(1, iy, iz)] for iy = 1:3 for iz = 1:3]
    xmax_surfs = [surfs_x[(4, iy, iz)] for iy = 1:3 for iz = 1:3]
    ...
    # y-periodicity: y_max surfaces are periodic copies of y_min
    ymin_surfs = [surfs_y[(ix, 1, iz)] for ix = 1:3 for iz = 1:3]
    ymax_surfs = [surfs_y[(ix, 4, iz)] for ix = 1:3 for iz = 1:3]
    gmsh.model.mesh.setPeriodic(
        2,
        ymax_surfs,
        ymin_surfs,
