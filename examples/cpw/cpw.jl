...
function plot_s_parameters(path::String, prefix::String="")
    ...
    for s_param in s_params
        ...
        # ax.limits = (nothing, (y_min, 1))

        # Adjust legend position based on data
        legend_position = y_min < -40 ? :lt : :rb
        axislegend(ax, position=legend_position, textsize=18)

        suffix = s_param[[3, 6]]
        filename = isempty(prefix) ? "cpw_$(suffix).png" : "cpw_$(prefix)_$(suffix).png"
