...
from palace.utils import create_basic_config, save_config, load_config


def create_cavity_eigenmode_config(mesh_file="cavity.msh",
                                 target_freq=5e9,
                                 num_modes=10):
    """
...
def create_dielectric_resonator_config(mesh_file="dielectric_resonator.msh",
                                     epsilon_r=10.0,
                                     loss_tan=1e-4):
    """
...
def plot_eigenmode_analysis(results, title="Eigenmode Analysis"):
    ...
    for i, (f, mt) in enumerate(zip(frequencies, mode_types)):
        axes[0, 0].text(i + 1, f / 1e9 + 0.1, mt,
    ...
    if q_factors is not None:
        ...
        for i, q in enumerate(q_factors):
            axes[0, 1].text(i + 1, q + max(q_factors) * 0.01, f'{q:.0f}',
                           ha='center', fontsize=8)
    else:
        axes[0, 1].text(0.5, 0.5, 'Q factors not available',
                       ha='center', va='center', transform=axes[0, 1].transAxes)
        axes[0, 1].set_title('Quality Factors')

    # Plot 3: Mode density
    axes[1, 0].hist(frequencies / 1e9, bins=min(10, len(frequencies) // 2 + 1),
    ...
    if q_factors is not None:
        scatter = axes[1, 1].scatter(frequencies / 1e9, q_factors,
                                   s=100, alpha=0.7, c=range(len(frequencies)))
        ...
        if len(frequencies) > 2:
            ...
            p = np.poly1d(z)
            axes[1, 1].plot(frequencies / 1e9, p(frequencies / 1e9),
                          "r--", alpha=0.8, label=f'Trend: Q = {z[0]:.0f}f + {z[1]:.0f}')
            axes[1, 1].legend()
    else:
        axes[1, 1].text(0.5, 0.5, 'Q factors not available',
...
def main():
    ...
    cavity_config = create_cavity_eigenmode_config(
        mesh_file="rectangular_cavity.msh",
        target_freq=5e9,
        num_modes=10
    )
    ...
    dr_config = create_dielectric_resonator_config(
        mesh_file="dielectric_resonator.msh",
        epsilon_r=10.0,
        loss_tan=1e-4
    )
    ...
    results_summary = {
        ...
        'q_factors': results['q_factors'].tolist() if results['q_factors'] is not None else None,
        'mode_types': results['mode_types'].tolist(),
        'analysis_notes': {
            'num_modes': results['num_modes'],
            'freq_range_GHz': [float(results['frequencies'][0] / 1e9),
                              float(results['frequencies'][-1] / 1e9)],
            'avg_mode_spacing_MHz': float(np.mean(np.diff(results['frequencies'])) / 1e6)
        }
    }
    ...
    for eps_r, mat_name in zip(epsilon_values, material_names):
        ...
        print(f"  {mat_name} (εᵣ={eps_r}): f₀ ≈ {scaled_freq/1e9:.2f} GHz → {config_file}")
