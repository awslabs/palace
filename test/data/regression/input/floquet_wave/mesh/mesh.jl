...
function generate_floquet_wave_mesh(;
    ...
    for iz = 2:3, iy = 1:3, ix = 1:3
        sz[(ix, iy, iz)] = make_surface(
            lx[(ix, iy, iz)],
            ly[(ix + 1, iy, iz)],
            -lx[(ix, iy + 1, iz)],
            -ly[(ix, iy, iz)]
    ...
    for iy = 2:3
        sy[(2, iy, 1)] =
            make_surface(lx[(2, iy, 1)], lz[(3, iy, 1)], -lx[(2, iy, 2)], -lz[(2, iy, 1)])
    end
    ...
    xmin_surfs = [sx[(1, iy, 2)] for iy = 1:3]
    xmax_surfs = [sx[(4, iy, 2)] for iy = 1:3]
    ymin_surfs = [sy[(ix, 1, 2)] for ix = 1:3]
    ymax_surfs = [sy[(ix, 4, 2)] for ix = 1:3]
    ...
    gmsh.model.addPhysicalGroup(
        2,
        [sz[(ix, iy, 3)] for iy = 1:3 for ix = 1:3],
        5,
        "floquet_port"
    )
    ...
    pec_surfs = [sz[(ix, iy, 2)] for iy = 1:3 for ix = 1:3 if !(ix == 2 && iy == 2)]
