                          ...
                          const std::vector<int> &attributes)
{
  ...
  Units units(1., 1.); // units(0.496, 1.453);  // Pick some arbitrary non-trivial units for testing
  IoData iodata{units};
  iodata.domains.materials.emplace_back().attributes = {1};
  ...
  // Debug frequency information
  // std::cout << "\n=== Frequency Debug Information ===";
  // std::cout << "\nInput frequency: " << freq_Hz / 1e6 << " MHz";
  // std::cout << "\nFreq nondimensionalized: " << units.Nondimensionalize<Units::ValueType::FREQUENCY>(freq_Hz);
  // std::cout << "\nPalace nondimensional omega (2π×freq_nd): " << omega;
  // std::cout << "\nPalace dimensional omega: " << units.Dimensionalize<Units::ValueType::FREQUENCY>(omega) << " rad/s";
  // std::cout << "\nAnalytical omega: " << 2.0 * M_PI * freq_Hz << " rad/s";
  // std::cout << "\nFrequency ratio (Palace/Analytical): " << units.Dimensionalize<Units::ValueType::FREQUENCY>(omega) / (2.0 * M_PI * freq_Hz);
  // std::cout << "\nWavelength: " << c0_ / freq_Hz << " m";
  // std::cout << "\nk = 2πf/c: " << 2.0 * M_PI * freq_Hz / c0_ << " rad/m";
  // std::cout << "\n====================================\n";
  ...
  std::vector<std::pair<mfem::Vector, std::string>> test_points = {
    {mfem::Vector({0.1, 0.0, 0.0}), "Near origin on x-axis"},
    {mfem::Vector({0.5, 0.0, 0.0}), "0.5m from origin on x-axis"},
    {mfem::Vector({1.0, 0.0, 0.0}), "1m from origin on x-axis"},
    {mfem::Vector({0.0, 0.0, 1.0}), "1m from origin on z-axis (dipole axis)"},
    {mfem::Vector({2.0, 0.0, 0.0}), "Far field on x-axis"},
    {mfem::Vector({0.0, 2.0, 0.0}), "Far field on y-axis"},
  };
  ...
  for (const auto &[pt, description] : test_points)
  {
    ...
    double theta = std::acos(pt(2) / r_nondim);
    double phi = std::atan2(pt(1), pt(0));

    // Compute dimensional field first (before nondimensionalization)
    double k = 2.0 * M_PI * freq_Hz / c0_;
    double kr = k * r_dim;
    double factor = p0 / (4.0 * M_PI * epsilon0_ * r_dim * r_dim * r_dim);
    std::complex<double> jkr(0, kr);
    std::complex<double> exp_jkr = std::exp(jkr);
    std::complex<double> Er_dim = factor * 2.0 * std::cos(theta) * (1.0 - jkr) * exp_jkr;
    std::complex<double> Etheta_dim = factor * std::sin(theta) * (1.0 - jkr - kr * kr) * exp_jkr;

    // Convert to Cartesian (dimensional)
    std::complex<double> Ex_dim = Er_dim * std::sin(theta) * std::cos(phi) + Etheta_dim * std::cos(theta) * std::cos(phi);
    std::complex<double> Ey_dim = Er_dim * std::sin(theta) * std::sin(phi) + Etheta_dim * std::cos(theta) * std::sin(phi);
    std::complex<double> Ez_dim = Er_dim * std::cos(theta) - Etheta_dim * std::sin(theta);

    // Evaluate nondimensional analytical solution
    auto E_analytical = ComputeDipoleENonDim(pt, units, p0, freq_Hz);

    double analytical_magnitude = std::sqrt(
      std::real(E_analytical[0]*std::conj(E_analytical[0])) +
    ...
    E_palace_real = 0.0; E_palace_imag = 0.0;
    ...
    double palace_magnitude = std::sqrt(
      E_palace_real(0)*E_palace_real(0) + E_palace_real(1)*E_palace_real(1) + E_palace_real(2)*E_palace_real(2) +
      E_palace_imag(0)*E_palace_imag(0) + E_palace_imag(1)*E_palace_imag(1) + E_palace_imag(2)*E_palace_imag(2)
    );

    std::cout << "\n" << description << " r=" << r_dim << "m:";
    std::cout << "\n  θ=" << theta*180/M_PI << "°, φ=" << phi*180/M_PI << "°, kr=" << kr;
    std::cout << "\n  PALACE FEM SOLUTION:";
    if (palace_eval_success) {
      std::cout << "\n    Palace |E|: " << palace_magnitude;
      std::cout << "\n    Palace Ex: " << std::complex<double>(E_palace_real(0), E_palace_imag(0));
      std::cout << "\n    Palace Ey: " << std::complex<double>(E_palace_real(1), E_palace_imag(1));
      std::cout << "\n    Palace Ez: " << std::complex<double>(E_palace_real(2), E_palace_imag(2));
    } else {
    ...
    if (palace_eval_success && analytical_magnitude > 1e-12) {
      std::cout << "\n  COMPARISON:";
      std::cout << "\n    |E| ratio (Palace/Analytical): " << palace_magnitude / analytical_magnitude;
    }
    std::cout << "\n  Field decay: |E| ~ 1/r^" << (r_dim > 0.5 ? std::log(analytical_magnitude)/std::log(1.0/r_dim) : 0.0);
  }
  std::cout << "\n============================================\n";
  ...
  std::cout << "\n\n--- Relative L2 Errors ---";
  std::cout << "\nReal part: || E_h_Re - E_Re || / ||E_Re|| = " << error_real / exact_norm_real;
  std::cout << "\nImag part: || E_h_Im - E_Im || / ||E_Im|| = " << error_imag / exact_norm_imag;
