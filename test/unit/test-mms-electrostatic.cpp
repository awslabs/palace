...
void CheckOptimalConvergence(const MmsCase &homogeneous, const MmsCase &nonhomogeneous)
{
  // Check every consecutive refinement pair for the expected p+1 potential rate and p
  // electric-field rate, for both homogeneous and non-homogeneous Dirichlet data.
  const std::array<std::pair<const char *, const MmsCase *>, 2> cases = {
      std::pair{"homogeneous", &homogeneous},
      std::pair{"non-homogeneous", &nonhomogeneous}};
  for (const auto &[name, mms] : cases)
  {
    for (const int order : kOrders)
    {
      DYNAMIC_SECTION(name << ", order " << order)
      {
...
void CheckCurvedConvergence(const MmsCase &curved)
{
  ...
  {
    error[i] = SolveMmsError(curved, LoadCurvedCylinderMesh(refinement[i]), order);
  }
