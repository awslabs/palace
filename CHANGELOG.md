...
## In progress
  ...
  - Changed csv output to print 17 digits for full double precision.
  ...
  - Added mutli-excitation support. `Excitation` in `LumpedPort` and `WavePort` boundary conditions
    ...
    excitation index.
  - Changed `RomOperator` to split out rational interpolation component into separate
    `MinimalRationInterpolation` class to support multiple excitations. THe MRI is unique to each
