...
//     `oneOf`. We opt into oneOf via `palace::schema::utils::schema_composition` at the
...
struct Eigenmode
{
  ...
  PALACE_SCHEMA_DESC(N, "Number of eigenvalues to compute.",
                     palace::schema::utils::XMin<int, 0>) = 1;
  ...
  PALACE_SCHEMA_DESC_ADVANCED(StartVector, "Use a random start vector for the eigensolver.",
                              bool) = true;
  ...
  PALACE_SCHEMA_DESC_ADVANCED(
      RefineNonlinear, "Refine nonlinear eigenvalue solutions after the initial solve.",
      bool) = true;
...
struct Linear
{
  ...
  PALACE_SCHEMA_DESC_ADVANCED(MGUseMesh,
...
struct Solver
{
  ...
  PALACE_SCHEMA_DESC_ADVANCED(
      ...
      "Use the Jacobian-based quadrature order instead of the default.", bool) = false;
