...
void inject_properties(rfl::Generic::Object& schema_entry,
    ...
    if (schema_entry.count("properties") == 0) return;
    auto& props_var = schema_entry.at("properties").value();
    if (!std::holds_alternative<rfl::Generic::Object>(props_var)) return;
    auto& props = std::get<rfl::Generic::Object>(props_var);

    for (auto& [name, prop_generic] : props) {
        if (!std::holds_alternative<rfl::Generic::Object>(
                prop_generic.value())) {
            continue;
        }
    ...
    if (root_obj.count("$defs") != 0) {
        ...
        if (std::holds_alternative<rfl::Generic::Object>(defs_var)) {
            ...
            for (auto& [def_name, def_generic] : defs) {
                ...
                if (it == defaults_map.end()) continue;
                if (!std::holds_alternative<rfl::Generic::Object>(
                        def_generic.value())) {
                    continue;
                }
                auto& def_obj =
                    std::get<rfl::Generic::Object>(def_generic.value());
                ...
                const auto& parsed_map =
                    std::get<rfl::Generic::Object>(parsed->value());
...
std::string inject_tag_descriptions(
    std::string schema_json,
    std::string tag_name,
    ...
    for (auto& [_def_name, def_g] : defs) {
        ...
        if (props.count(tag_name) == 0) continue;
        auto& tag_prop_var = props.at(tag_name).value();
        if (!std::holds_alternative<rfl::Generic::Object>(tag_prop_var)) continue;
        ...
        // field named `Type`.
        if (tag_prop.count("enum") == 0) continue;
        auto& enum_var = tag_prop.at("enum").value();
        if (!std::holds_alternative<rfl::Generic::Array>(enum_var)) continue;
        auto& enum_arr = std::get<rfl::Generic::Array>(enum_var);
        if (enum_arr.size() != 1) continue;
        if (!std::holds_alternative<std::string>(enum_arr[0].value())) continue;
        const auto& discriminator = std::get<std::string>(enum_arr[0].value());

        for (const auto& [disc, desc] : arm_descs) {
            if (disc != discriminator) continue;
            if (tag_prop.count("description") != 0) break;
...
std::string prune_required(
    ...
    if (prune_map.empty()) return schema_json;
    auto schema_gen_r = rfl::json::read<rfl::Generic>(schema_json);
    if (!schema_gen_r) return schema_json;
    ...
    for (auto& [def_name, def_g] : defs) {
        ...
        if (it == prune_map.end() || it->second.empty()) continue;
        if (!std::holds_alternative<rfl::Generic::Object>(def_g.value())) {
            continue;
        }
        auto& def_obj = std::get<rfl::Generic::Object>(def_g.value());
        if (def_obj.count("required") == 0) continue;
        auto& req_var = def_obj.at("required").value();
        if (!std::holds_alternative<rfl::Generic::Array>(req_var)) continue;
...
const rfl::Generic* resolve_ref(const rfl::Generic::Object& root_obj,
                                std::string_view ref) {
    ...
    if (root_obj.count("$defs") == 0) return nullptr;
    const auto& defs_var = root_obj.at("$defs").value();
    if (!std::holds_alternative<rfl::Generic::Object>(defs_var)) return nullptr;
    const auto& defs = std::get<rfl::Generic::Object>(defs_var);
    if (defs.count(name) == 0) return nullptr;
    return &defs.at(name);
}

bool is_null_schema(const rfl::Generic& node) {
    if (!std::holds_alternative<rfl::Generic::Object>(node.value())) return false;
    const auto& obj = std::get<rfl::Generic::Object>(node.value());
    if (obj.count("type") == 0) return false;
    const auto& t_var = obj.at("type").value();
    if (!std::holds_alternative<std::string>(t_var)) return false;
...
std::optional<std::pair<std::string, std::string>> arm_discriminator(
    const rfl::Generic::Object& root_obj, const rfl::Generic& arm_schema) {
    ...
    if (obj.count("properties") == 0) return std::nullopt;
    const auto& props_var = obj.at("properties").value();
    if (!std::holds_alternative<rfl::Generic::Object>(props_var)) return std::nullopt;
    const auto& props = std::get<rfl::Generic::Object>(props_var);
    for (const auto& [name, body] : props) {
        if (!std::holds_alternative<rfl::Generic::Object>(body.value())) continue;
        const auto& body_obj = std::get<rfl::Generic::Object>(body.value());
        if (body_obj.count("enum") == 0) continue;
...
void check_node(const rfl::Generic::Object& root_obj,
                const rfl::Generic& schema_node,
                const rfl::Generic& input_node,
                std::string path,
                std::vector<std::string>& out) {
    if (!std::holds_alternative<rfl::Generic::Object>(schema_node.value())) return;
    ...
    if (schema_obj.count("$ref") != 0) {
        ...
        if (!std::holds_alternative<std::string>(ref_var)) return;
        const auto* referenced =
            resolve_ref(root_obj, std::get<std::string>(ref_var));
        if (referenced) {
    ...
    // anyOf: palace::schema::utils emits this only for `std::optional<T>` (= [T-schema, null]).
    // If input is null, the null branch matches — nothing to check. Else
    // recurse into the first non-null branch.
    if (schema_obj.count("anyOf") != 0) {
        if (std::holds_alternative<std::nullopt_t>(input_node.value())) return;
        const auto& arr_var = schema_obj.at("anyOf").value();
        if (!std::holds_alternative<rfl::Generic::Array>(arr_var)) return;
        const auto& arr = std::get<rfl::Generic::Array>(arr_var);
        for (const auto& branch : arr) {
            if (is_null_schema(branch)) continue;
    ...
    if (schema_obj.count("oneOf") != 0) {
        if (!std::holds_alternative<rfl::Generic::Object>(input_node.value())) return;
        ...
        for (const auto& branch : arr) {
            ...
            if (!disc) continue;
            if (input_obj.count(disc->first) == 0) continue;
            const auto& input_tag_var = input_obj.at(disc->first).value();
            if (!std::holds_alternative<std::string>(input_tag_var)) continue;
            if (std::get<std::string>(input_tag_var) != disc->second) continue;
            check_node(root_obj, branch, input_node, path, out);
            return;
        }
        return;
    ...
    if (schema_obj.count("items") != 0) {
        if (!std::holds_alternative<rfl::Generic::Array>(input_node.value())) return;
        ...
        for (std::size_t i = 0; i < input_arr.size(); ++i) {
            check_node(root_obj, items, input_arr[i],
    ...
    if (schema_obj.count("properties") == 0) return;
    if (!std::holds_alternative<rfl::Generic::Object>(input_node.value())) return;
    const auto& input_obj = std::get<rfl::Generic::Object>(input_node.value());
    ...
    if (schema_obj.count("required") != 0) {
        ...
        if (std::holds_alternative<rfl::Generic::Array>(req_var)) {
            ...
            for (const auto& r : req_arr) {
                if (!std::holds_alternative<std::string>(r.value())) continue;
    ...
    for (const auto& [name, input_val] : input_obj) {
        if (props.count(name) == 0) continue;  // extras — NoExtraFields rejects
...
std::vector<std::string> check_required_keys(const std::string& schema_json,
    ...
    if (!schema_r || !input_r) return {};
    if (!std::holds_alternative<rfl::Generic::Object>(schema_r->value())) return {};
...
std::string rewrite_root_composition(std::string schema_json,
    ...
    if (root_obj.count(from) == 0) return schema_json;
