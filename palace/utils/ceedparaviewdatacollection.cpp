                                                   ...
                                                   const CellField &field) const
{
  ...
  MFEM_VERIFY(field.values && field.num_comp > 0, "Invalid domain cell field registration!");
  ...
  for (int i = 0; i < mesh->GetNE(); i++)
  {
    ...
    {
      mfem::WriteBinaryOrASCII(os, buf, data[i * field.num_comp + c], " ",
                               pv_data_format);
    }
...
void CeedParaViewDataCollection::RegisterDomainCellField(
    const std::string &field_name, const Vector &values, int num_comp)
{
...
void CeedParaViewDataCollection::DeregisterDomainCellField(
    const std::string &field_name)
{
...
void CeedParaViewDataCollection::SaveDataVTU(std::ostream &os, int ref)
{
  ...
  else
  {
    ...
    os.write(mesh_vtu.data() + pos,
             static_cast<std::streamsize>(mesh_vtu.size() - pos));
  }
