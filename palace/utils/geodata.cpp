...
// Cleanup the provided serial mesh by removing unnecessary domain and elements and adding
// boundary elements for material interfaces and exterior boundaries.
std::map<int, std::array<int, 2>> CheckMesh(mfem::Mesh &, const IoData &, bool, bool);

// Add boundary elements for subdomain interfaces to the serial mesh.
std::map<int, std::array<int, 2>> AddSubdomainInterfaces(mfem::Mesh &,
                                        ...
                                        bool clean_elem, bool add_bdr, bool add_subdomain)
{
  ...
  if (Mpi::Root(comm) || !use_mesh_partitioner)
  {
    ...
    if (smesh->Conforming())
    {
      CheckMesh(*smesh, iodata, clean_elem, add_bdr);
    }
    ...
    {
      AddSubdomainInterfaces(*smesh, partitioning);
    }
  }
...
void FlipVertices(mfem::Element *el)
{
  mfem::Array<int> v;
  el->GetVertices(v);
...
void FinalizeMesh(const mfem::Mesh &orig_mesh, mfem::Mesh &new_mesh,
                  const mfem::Array<bool> *elem_delete = nullptr)
{
...
std::map<int, std::array<int, 2>> CheckMesh(mfem::Mesh &orig_mesh, const IoData &iodata,
                                            bool clean_elem, bool add_bdr)
{
  ...
  FinalizeMesh(orig_mesh, new_mesh, &elem_delete);
  orig_mesh = std::move(new_mesh);
  return new_attr_map;
}

std::map<int, std::array<int, 2>>
AddSubdomainInterfaces(mfem::Mesh &orig_mesh, const std::unique_ptr<int[]> &partitioning)
{
  // Add boundary elements for subdomain interfaces.
  ...
  mfem::Mesh new_mesh(orig_mesh.Dimension(), orig_mesh.GetNV(), orig_mesh.GetNE(), new_nbdr,
                      orig_mesh.SpaceDimension());
  ...
  for (int f = 0; f < orig_mesh.GetNumFaces(); f++)
  {
    if (add_bdr_faces[f] > 0)
    {
      ...
      MFEM_VERIFY(a + b > 0, "Invalid new boundary element attribute!");
      int new_attr =
          max_bdr_attr + (b > 0 ? (a * (a - 1)) / 2 + b : a);  // At least max_bdr_attr + 1
      if (new_attr_map.find(new_attr) == new_attr_map.end())
      {
        new_attr_map.emplace(new_attr, std::array<int, 2>{a, b});
      }
  ...
  FinalizeMesh(orig_mesh, new_mesh);
  orig_mesh = std::move(new_mesh);
  return new_attr_map;
}
