...
// Given a serial CONFORMAL mesh on the root processor, and element
// partitioning, create a parallel mesh over the given communicator.
                                        ...
                                        Timer &timer)
{
  ...
  //
  // If nonconformal adaptation is being performed,
  auto t0 = timer.Now();
  ...
  if (Mpi::Root(comm) || !use_mesh_partitioner)
  {
    ...
    // Validate the assumption that a nonconformal mesh is allowed.
...
std::unique_ptr<mfem::Mesh> LoadMesh(const std::string &path)
{
  ...
  auto mesh = std::make_unique<mfem::Mesh>(fi, 1, 1, true);
  mesh->EnsureNodes();
  return mesh;
}
                                            ...
                                            bool add_bdr, bool add_subdomain)
{
  ...
  // Perform Finalization last, as the true argument allows for element vertex
  // reordering, to ensure that the longest edge is first. This prepares for
