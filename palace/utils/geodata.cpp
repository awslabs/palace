...
std::unique_ptr<mfem::ParMesh> ReadMesh(MPI_Comm comm, const IoData &iodata)
{
  ...
  if (smesh)
  {
    ...
    if (smesh->Conforming())
    {
      std::vector<int> attr_list, bdr_attr_list, crack_bdr_attr_list;
      ...
      std::merge(iodata.boundaries.attributes.begin(), iodata.boundaries.attributes.end(),
                 iodata.boundaries.postpro.attributes.begin(),
                 iodata.boundaries.postpro.attributes.end(),
                 std::back_inserter(bdr_attr_list));
      ...
      CheckMesh(smesh, attr_list, bdr_attr_list, iodata.model.clean_unused_elements,
                crack_bdr_attr_list, iodata.model.add_bdr_elements);
    }
