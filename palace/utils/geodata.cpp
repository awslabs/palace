                                        ...
                                        bool clean_elem, bool add_bdr, bool add_subdomain, bool tear_internal_bdr)
{
  ...
  if (Mpi::Root(comm) || !use_mesh_partitioner)
  {
    ...
    if ((clean_elem || add_bdr || tear_internal_bdr) && smesh->Conforming())
    {
      static_cast<void>(CheckMesh(smesh, iodata, clean_elem, add_bdr, false, tear_internal_bdr, nullptr));
    }
    ...
    {
      static_cast<void>(
          CheckMesh(smesh, iodata, false, false, add_subdomain, false, partitioning.get()));
    }
  }
...
std::map<int, std::array<int, 2>> CheckMesh(std::unique_ptr<mfem::Mesh> &orig_mesh,
                                            ...
                                            bool tear_interior_boundaries,
                                            const int *partitioning)
{
  // tear_interior_boundaries = false;
  ...
  if (!clean_elem && !add_bdr && !add_subdomain && !tear_interior_boundaries)
  {
    return {};
  }
  ...
  int new_vertices = orig_mesh->GetNV();
  ...
  if (add_bdr || tear_interior_boundaries)
  {
    ...
    int add_bdr_ext = 0, add_bdr_int = 0, add_bdr_tear = 0;
    std::map<int, int> edge_visit_count;
    mfem::Array<int> edges, orientations, faces;
    for (int f = 0; f < orig_mesh->GetNumFaces(); f++)
    {
      const int &be = orig_bdr_faces[f];
      if (add_bdr && be < 0 && add_bdr_faces[f] < 0)
      {
        int e1, e2;
        orig_mesh->GetFaceElements(f, &e1, &e2);
        bool no_e1 = (e1 < 0 || elem_delete[e1]);
      ...
      if (tear_interior_boundaries && be >= 0 && add_bdr_faces[f] < 0)
      {
        // If it is a pre-existing interior boundary element, it should be torn
        int e1, e2;
        orig_mesh->GetFaceElements(f, &e1, &e2);
        bool no_e1 = (e1 < 0 || elem_delete[e1]);
        bool no_e2 = (e2 < 0 || elem_delete[e2]);
        if (!no_e1 && !no_e2)
        {
          // Mpi::Print("Tearing an interior boundary element!\n");
          add_bdr_tear++;
          orig_mesh->GetFaceEdges(f, edges, orientations);
          for (const auto &e : edges)
          {
            edge_visit_count[e]++; // Default initializes to zero
          }

          // Loop over other faces of attached elements. If they are an external boundary,
          // check if any edges of those faces are already in the map, if so increment the
          // counter. This is to check for boundary edges coincident to external boundaries.
          auto detect_exterior_edges = [&](int elem)
          {
            orig_mesh->GetElementFaces(elem, faces, orientations);
            for (const auto &ef : faces)
            {
              int eL, eR;
              orig_mesh->GetFaceElements(f, &eL, &eR);
              if (eR > 0) // Skip internal faces
              {
                continue;
              }
              orig_mesh->GetFaceEdges(ef, edges, orientations);

              for (const auto &e : edges)
              {
                std::cout << "elem " << elem << " has edge " << e << std::endl;
                if (edge_visit_count.find(e) != edge_visit_count.end())
                {
                  edge_visit_count[e]++;
                }
              }
          ...
          detect_exterior_edges(e1);
          detect_exterior_edges(e2);
        }
      }
      for (const auto &e: edge_visit_count)
      {
        if (e.second == 1)
        {
          orig_mesh->GetEdgeVertices(e.first, edges);
          seam_vertices.push_back(edges[0]);
          seam_vertices.push_back(edges[1]);
        }
      }
    }
    std::sort(seam_vertices.begin(), seam_vertices.end());
    seam_vertices.erase(std::unique(seam_vertices.begin(), seam_vertices.end()), seam_vertices.end());
    std::cout << "seam_vertices.size() " << seam_vertices.size() << std::endl;

    new_vertices += static_cast<int>(seam_vertices.size());
    new_nbdr += (add_bdr_ext + add_bdr_int + add_bdr_tear);
    if (add_bdr_ext > 0)
    {
    ...
    if (add_bdr_tear > 0)
    {
      Mpi::Print("Added {:d} boundary elements for torn internal boundaries to the mesh\n",
                 add_bdr_tear);
    }
  }
  ...
  auto new_mesh =
      std::make_unique<mfem::Mesh>(orig_mesh->Dimension(), new_vertices, new_ne,
                                   new_nbdr, orig_mesh->SpaceDimension());
  std::map<int, std::array<int, 2>> new_attr_map;

  // Copy original and duplicated vertices, and non-deleted domain and boundary elements.
  ...
  auto OriginalAttributes = [max_bdr_attr](int new_attr)
  {
    // Invert the Cantor pairing function, see
    // https://en.wikipedia.org/wiki/Pairing_function.
    new_attr -= max_bdr_attr;
    const int w = std::floor((std::sqrt(8*new_attr +1) - 1)/ 2);
    const int t = w*(w+1)/2;
    const int b = new_attr - t;
    const int a = w - b;
    return std::make_pair(a, b);
  };

  // Add in true external boundaries. This is necessary to be able to recognize seam
  // boundaries if tearing.
  if (add_bdr || add_subdomain)
  {
    // Some (1-based) boundary attributes may be empty since they were removed from the
    // original mesh, but to keep attributes the same as config file we don't compress the
    ...
    for (int f = 0; f < orig_mesh->GetNumFaces(); f++)
    {
      if (add_bdr_faces[f] > 0)
      {
        ...
        if (add_bdr_faces[f] > 1)
        {
          ...
          auto *el = orig_mesh->GetFace(f)->Duplicate(new_mesh.get());
          FlipVertices(el);
          el->SetAttribute(new_attr);
          new_mesh->AddBdrElement(el);
          if constexpr (false)
          {
            auto [a, b] = OriginalAttributes(new_attr);
            Mpi::Print(
                "Adding two boundary elements with attr {:d} from element attributes {:d} and {:d}\n",
                new_attr, a, b);
          }
        }
      }
  ...
  // Add new boundary elements corresponding to torn boundaries.
  if (tear_interior_boundaries)
  {
    const int initial_nvert = orig_mesh->GetNV();
    const int initial_nbe = orig_mesh->GetNBE();
    mfem::Array<int> vertices;
    std::map<int, int> old_to_new_v;
    for (int i = 0; i < static_cast<int>(seam_vertices.size()); i++)
    {
      old_to_new_v[seam_vertices[i]] = i + initial_nvert;
    }
    for (int be = 0; be < orig_mesh->GetNBE(); be++)
    {
      int f, o, e1, e2;
      ...
      bool no_e1 = (e1 < 0 || elem_delete[e1]);
      bool no_e2 = (e2 < 0 || elem_delete[e2]);
      if (no_e1 || no_e2) // Is this an internal boundary?
      {
        continue;
      }

      // Create a duplicate of the boundary element
      // Change the vertices on the new boundary element to the duplicates.
      // Reverse so the normal is correct.
      auto *new_elem = orig_mesh->GetBdrElement(be)->Duplicate(new_mesh.get());
      new_elem->GetVertices(vertices);
      for (auto &v : vertices)
      {
        v = old_to_new_v[v];
      }
      std::reverse(vertices.begin(), vertices.end());
      new_elem->SetVertices(vertices);
      new_mesh->AddBdrElement(new_elem);

      // Change the vertices for the second element. All the new elements need to be on the
      // same side of the introduced crack. The higher attribute element is attached to the
      // new boundary element.
      auto *elem_1 = new_mesh->GetElement(e1);
      auto *elem_2 = new_mesh->GetElement(e2);
      if (elem_1->GetAttribute() > elem_2->GetAttribute())
      {
        elem_1->GetVertices(vertices);
        for (int i = 0; i < vertices.Size(); i++)
        {
          vertices[i] = old_to_new_v[vertices[i]];
        }
        elem_1->SetVertices(vertices);
      }
      else if (elem_1->GetAttribute() < elem_2->GetAttribute())
      {
        elem_2->GetVertices(vertices);
        for (int i = 0; i < vertices.Size(); i++)
        {
          vertices[i] = old_to_new_v[vertices[i]];
        }
        elem_2->SetVertices(vertices);
      }
      else
      {
        // TODO: An alternative tie-break for interior boundaries within a volume of the
        // same attribute?
        MFEM_ABORT("Interior boundary between same volume attribute, cannot tie break crack placement!");
      }
    }
  }
  ...
  int orig_pec_attr = 0, new_pec_attr = 0;
  for (int be = 0; be < orig_mesh->GetNBE(); be++)
  {
    if (orig_mesh->GetBdrAttribute(be) == 13)
    {
      orig_pec_attr++;
    }
  }

  for (int be = 0; be < new_mesh->GetNBE(); be++)
  {
    if (new_mesh->GetBdrAttribute(be) == 13)
    {
      new_pec_attr++;
    }
  }

  std::cout << "orig_pec_attr " << orig_pec_attr << " new_pec_attr " << new_pec_attr << std::endl;
