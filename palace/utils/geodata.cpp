  ...
  if (smesh)
  {
    ...
    if (smesh->Conforming())
    {
      ...
      if (crack_bdr_elem)
      {
        ...
        // XX TODO SELECT ATTRIBUTES FOR CRACKING...
               ...
               bool add_bdr_elem)
{
  ...
  // - If desired, create a new mesh which has added boundary elements for all exterior
  //   boundaries and material interfaces if these elements do not yet exist.
  // - If desired, create a new mesh where internal boundary elements corresponding to
  //   certain attributes are "cracked", decoupling the elements on either side.
  ...
  auto bdr_marker = mesh::AttrToMarker(
      orig_mesh->bdr_attributes.Size() ? orig_mesh->bdr_attributes.Max() : 0, bdr_attr_list,
      true);
  {
  ...
  if (!crack_bdr_attr_list.empty())
  {
    ...
    for (int be = 0; be < orig_mesh->GetNBE(); be++)
    {
          ...
          bdr_elem_delete_map[be] >= 0)
      {
        ...
        if (!no_e1 && !no_e2)
        {
          // Crack elements are deleted here, and added the pair of new ones is added later.
          bdr_elem_delete_map[be] = -1;
          crack_bdr_elem.insert(be);
        }
      }
    }
    {
      // Finalize bdr_elem_delete_map updates.
      int count = 0;
      for (int be = 0; be < orig_mesh->GetNBE(); be++)
      {
        if (bdr_elem_delete_map[be] >= 0)
        {
          bdr_elem_delete_map[be] = count++;
        }
      }
    }
    ...
    {
      const int *verts = orig_mesh->GetBdrElement(be)->GetVertices();
      for (int i = 0; i < orig_mesh->GetBdrElement(be)->GetNVertices(); i++)
      {
        ...
        unvisited.reserve(vert_to_elem->RowSize(v));
        for (int j = 0; j < vert_to_elem->RowSize(v); j++)
        {
          if (elem_delete_map[elems[j]] >= 0)
          {
            unvisited.insert(elems[j]);
          }
        }
        const auto nbr_size = unvisited.size();
        ...
        while (!unvisited.empty())
        {
          ...
          while (!que.empty())
          {
            ...
            for (int j = 0; j < elem_to_face.RowSize(e); j++)
            {
              ...
              {
                const auto it = face_to_be.find(f);
              ...
              int nbr = (e == e1) ? e2 : e1;
              const auto it = unvisited.find(nbr);
              if (nbr >= 0 && elem_delete_map[nbr] >= 0 && it != unvisited.end())
              {
                que.push(nbr);
                unvisited.erase(it);
            ...
            "No connected components found for elements adjacent to a crack vertex!");
        {
          ...
          MFEM_VERIFY(visited_size == nbr_size,
        ...
        // renumbering so is not saved. We also store a set of all modified domain elements
        // around the crack.
        if (components.size() > 1)
        {
          ...
          {
            vert_components.emplace_back(orig_mesh->GetNV() + new_nv_dups, std::move(*it));
            new_nv_dups++;
          }
        }
      }
    ...
    if (DEBUG_OUTPUT)
    {
      std::cout << "\nIdentified " << crack_vert_duplicates.size()
  ...
  // Create the new mesh.
  if (new_nv == orig_mesh->GetNV() && new_ne == new_ne_step_2 &&
      new_ne_step_2 == new_ne_step_1 && new_ne_step_1 == orig_mesh->GetNE() &&
      new_nbe == new_nbe_step_2 && new_nbe_step_2 == new_nbe_step_1 &&
      new_nbe_step_1 == orig_mesh->GetNBE())
  {
    return;
  }
  ...
  // Add duplicated vertices from interior boundary cracking, renumber domain and boundary
  // elements to tear the mesh, and add new crack boundary elements.
  if (!crack_bdr_attr_list.empty())
  {
    // Add duplicate vertices.
    for (const auto &[orig_v, vert_components] : crack_vert_duplicates)
    {
      for (std::size_t i = 0; i < vert_components.size(); i++)
      {
        new_mesh->AddVertex(orig_mesh->GetVertex(orig_v));
      }
    }
    ...
    if (DEBUG_OUTPUT)
    {
      std::cout << "\nAdded " << new_mesh->GetNV() - orig_mesh->GetNV()
                << " vertices for internal boundary cracking\n\n";
    }
      ...
      {
        {
          ...
          for (int i = 0; i < vert_to_elem->RowSize(orig_v); i++)
          {
            ...
            const auto new_e = elem_delete_map[e];
            if (new_e < 0)
            {
              continue;
            }
            int *verts = new_mesh->GetElement(new_e)->GetVertices(), j;
            for (j = 0; j < new_mesh->GetElement(new_e)->GetNVertices(); j++)
            {
              if (verts[j] == orig_v)
              {
                break;
            ...
            MFEM_VERIFY(j < new_mesh->GetElement(new_e)->GetNVertices(),
                        "Unable to locate vertex in element!");
            ...
            for (const auto &[dup_v, component] : vert_components)
            {
              if (component.find(e) != component.end())
              {
                ...
                if (DEBUG_OUTPUT)
                {
                  std::cout << "Renumbering vertex " << orig_v << " to " << dup_v
                            << " in element " << e << "\n";
                }
        ...
        }
        {
          ...
          {
            // Find vertex in the boundary element.
            if (bdr_elem_delete_map[bdr_elems[i]] < 0)
            {
              continue;
            }
            const auto be = bdr_elems[i];
            const auto new_be = bdr_elem_delete_map[be];
            int *verts = new_mesh->GetBdrElement(new_be)->GetVertices(), j;
            for (j = 0; j < new_mesh->GetBdrElement(new_be)->GetNVertices(); j++)
            {
              if (verts[j] == orig_v)
              {
                break;
            ...
            MFEM_VERIFY(j < new_mesh->GetBdrElement(new_be)->GetNVertices(),
                        "Unable to locate vertex in element!");

            // Find the correct duplicate for this vertex. Use the neighboring elements,
            // which must be part of the same connected component. Again, it is OK if the
            // element is not in any of the connected components, this indicates that it
            // keeps the original vertex and its connectivity is unmodified.
            int f, o, e1, e2;
            orig_mesh->GetBdrElementFace(be, &f, &o);
            orig_mesh->GetFaceElements(f, &e1, &e2);
            bool no_e1 = (e1 < 0 || elem_delete_map[e1] < 0);
            bool no_e2 = (e2 < 0 || elem_delete_map[e2] < 0);
            MFEM_VERIFY(!(no_e1 && no_e2), "Boundary element with no attached elements!");
            int e = no_e1 ? e2 : e1;
            for (const auto &[dup_v, component] : vert_components)
            {
              if (component.find(e) != component.end())
    ...
    // Finally, we insert the new boundary elements for the crack interface.
    const mfem::Table &elem_to_face = orig_mesh->ElementToFaceTable();
    for (auto be : crack_bdr_elem)
    {
      int f, o, e1, e2;
      orig_mesh->GetBdrElementFace(be, &f, &o);
      orig_mesh->GetFaceElements(f, &e1, &e2);
      MFEM_VERIFY(e1 >= 0 && elem_delete_map[e1] >= 0 && e2 >= 0 &&
                      elem_delete_map[e2] >= 0,
                  "Incorrect labeling of interior crack boundary element!");
      for (int e : {e1, e2})
      {
        ...
        if (DEBUG_OUTPUT)
        {
          std::cout << "Added crack interface boundary element with vertices:";
          for (int j = 0; j < bdr_el->GetNVertices(); j++)
          {
            std::cout << " " << bdr_el->GetVertices()[j];
          }
          std::cout << "\n";
        }
      }
