...
{
public:
  // A component-major scalar subrange of a packed boundary point-field result. The
  // descriptor is intentionally narrow: batches are only supported for boundary output,
  // where every member shares one fixed sampling lattice.
  struct BoundaryPointFieldDescriptor
  {
    std::string name;
    int offset = 0;
    int size = 0;
...
private:
  struct PointFieldProvider
  {
    std::function<const Vector &()> evaluator;
    const Vector *cached_values = nullptr;
    int expected_size = 0;
  };
  struct PointField
  {
    ...
    std::shared_ptr<PointFieldProvider> provider;
  ...
  void ResetPointFieldProviders();
  const Vector &GetPointFieldValues(MeshEntityType location, const PointField &field,
                                    Vector &view, Vector *scratch) const;
...
public:
  ...
  void RegisterBoundaryPointEvaluator(const std::string &field_name,
                                      ...
                                      const std::vector<int> &bases, int num_comp,
                                      int buffer_size);
  // Register views into one packed, component-major boundary result. The provider is
  // called at most once per SaveDataVTU invocation and its returned Vector must retain
  // its storage until that synchronous save completes.
  void
  RegisterBoundaryPointFieldBatch(const std::vector<BoundaryPointFieldDescriptor> &fields,
                                  const std::vector<int> &bases,
                                  std::function<const Vector &()> provider,
                                  int expected_packed_size);
  ...
  // from shared provider calls and account exactly for non-owning packed slice views.
  static long long IndividualEvaluatorCallCount();
  static long long ProviderCallCount();
...
private:
  static std::atomic<long long> individual_evaluator_calls;
  static std::atomic<long long> provider_calls;
  static std::atomic<long long> batch_view_fields;
  static std::atomic<long long> avoided_slice_copies;
  static std::atomic<long long> avoided_slice_copy_bytes;
};
