                                                ...
                                                const FiniteElementSpace &nd_fespace)
{
    ...
    surface_attrs.insert(surface_attrs.end(), 
                        flux_data.hole_attributes.begin(), 
    ...
    const_cast<mfem::ParFiniteElementSpace*>(fespace)->GetBoundaryElementsByAttribute(hole_surface_attrs, attr_to_elements);
    for (int h = 0; h < num_holes; h++)
    {
        const_cast<mfem::ParFiniteElementSpace*>(fespace)->GetBoundaryEdgeDoFs(attr_to_elements[hole_surface_attrs[h]], hole_ess_tdof_lists[h], 
    ...
    mfem::ParSubMesh boundary_submesh = mfem::ParSubMesh::CreateFromBoundary(pmesh, bdr_attr_marker);
    ...
    mesh::MatchBoundaryEdges(pmesh, boundary_submesh, submesh_boundary_edge_ids,
                             submesh_to_parent_bdr_edge_map, hole_dof_to_edge_maps, hole_boundary_edges);
    // Assign boundary attributes and create edge sets
    int current_attr = boundary_submesh.bdr_attributes.Size() > 0 ? boundary_submesh.bdr_attributes[0] : 1;
    ...
    for (int h = 0; h < num_holes; h++) {
        ...
        hole_edge_sets[h] = std::unordered_set<int>(hole_boundary_edges[h].begin(), hole_boundary_edges[h].end());
    }
    ...
    for (int h = 0; h < num_holes; h++)
    {
        ...
        perimeter_form.AddBoundaryIntegrator(new mfem::BoundaryLFIntegrator(one), hole_bdr_markers[h]);
        perimeter_form.Assemble();
        
        double local_perimeter = perimeter_form.Sum();
        MPI_Allreduce(&local_perimeter, &hole_perimeters[h], 1, MPI_DOUBLE, MPI_SUM, boundary_submesh.GetComm());
        
        hole_field_values[h] = flux_values[h] / hole_perimeters[h];
        
        std::unordered_map<int, int> hole_edge_orientations;
        mesh::ComputeSubmeshBoundaryEdgeOrientations(boundary_submesh, hole_boundary_edges[h], loop_normal, 
                                                     hole_edge_orientations, hole_edge_lengths[h]);
        for (auto& pair : hole_edge_lengths[h]) pair.second = -pair.second;
    }
    ...
    submesh_nd_fecs.push_back(std::make_unique<mfem::ND_FECollection>(order, boundary_submesh.Dimension()));
    std::vector<std::unique_ptr<mfem::H1_FECollection>> submesh_h1_fecs;
    submesh_h1_fecs.push_back(std::make_unique<mfem::H1_FECollection>(order, boundary_submesh.Dimension()));
    std::vector<std::unique_ptr<Mesh>> submesh_vec;
    submesh_vec.push_back(std::make_unique<Mesh>(boundary_submesh, 1));
    
    FiniteElementSpaceHierarchy submesh_nd_fespaces(fem::ConstructFiniteElementSpaceHierarchy<mfem::ND_FECollection>(
        1, submesh_vec, submesh_nd_fecs, nullptr, nullptr));
    FiniteElementSpaceHierarchy submesh_h1_fespaces(fem::ConstructFiniteElementSpaceHierarchy<mfem::H1_FECollection>(
        1, submesh_vec, submesh_h1_fecs, nullptr, nullptr));
    
    mfem::Array<int> submesh_ess_tdof_list;    
    submesh_nd_fespaces.GetFESpaceAtLevel(0).Get().GetEssentialTrueDofs(combined_inner_bdr_marker, submesh_ess_tdof_list);

    // First synchronize the marker itself to ensure all processors agree on which DoFs to sync
    mfem::GroupCommunicator *gc = submesh_nd_fespaces.GetFESpaceAtLevel(0).Get().ScalarGroupComm();
    ...
    gc->ReduceMarked<double>(values.GetData(), global_marker, 0, mfem::GroupCommunicator::MaxAbs<double>);
    ...
    auto K_op = std::make_unique<ParOperator>(std::move(k_vec[0]), submesh_nd_fespaces.GetFESpaceAtLevel(0));
                           ...
                           MPI_Comm comm)
{
    for (int h = 0; h < static_cast<int>(hole_attributes.size()); h++) {
        ...
        if (rank == 0) {
            Mpi::Print("Hole attribute {:d}: Target flux = {:.6e}, Computed flux = {:.6e}, Error = {:.6e}\n", 
