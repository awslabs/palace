...
#include "newmodeanalysissolver.hpp"
...
NewModeAnalysisSolver::Solve(const std::vector<std::unique_ptr<Mesh>> &mesh) const
{
  MFEM_VERIFY(mesh.back()->Dimension() == 2,
              "NewModeAnalysis solver requires a 2D mesh (waveguide cross-section)!");
  ...
  Mpi::Print("\nConfiguring 2D waveguide new linear mode analysis at f = {:.3e} GHz "
             "(omega = {:.6e})\n",
             freq_GHz, omega);
  ...
  // Construct the new linear boundary mode solver.
  NewBoundaryModeSolver mode_solver(config, nd_fespace, h1_fespace, dbc_tdof_list,
                                    mesh.back()->GetComm());
  ...
  Mpi::Print("\nSolving new linear GEP for {:d} propagation mode(s)...\n", num_modes);
  ...
  for (int i = 0; i < num_conv; i++)
  {
    ...
    Mpi::Print(" [New] eig {:d}: lambda = {:.6e}{:+.6e}i, kn = {:.6e}{:+.6e}i, "
               "n_eff = {:.6e}{:+.6e}i\n",
