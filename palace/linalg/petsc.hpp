...
//   // Sets all entries of the vector to random numbers sampled from the range [-1-i, 1+i],
//   or
//   // [-1, 1].
//   void SetRandom();
// #if defined(PETSC_USE_COMPLEX)
...
//   // assumed the structure does not change for the lifetime of the matrix(unless
//   explicitly
//   // set again).
...
//   // corresponding to the provided rows and columns which are the selected(local)
//   indices. virtual std::unique_ptr<PetscParMatrix> GetSubMatrix(const mfem::Array<int>
//   &rows,
//                                                        const mfem::Array<int> &cols);

//   // Create a sequential gathered matrix corresponding to the parallel matrix. All
//   processes
//   // on the original communicator must call this function, but if the argument is false,
//   no
...
// public:
//   // Create a parallel or sequential PETSc dense matrix. Option to specify an existing
//   data
//   // array.
...
//   // Set the (local) matrix dimensions to m x n, copying previous contents to the
//   upper-left
//   // block.
...
//   // Sets all entries of the matrix to random numbers sampled from the range [-1-i, 1+i],
//   or
//   // [-1, 1].
...
//   // Orthonormalize column j of the matrix against the preceeding columns, using
//   classical
//   // or modified Gram-Schmidt.
//   PetscReal OrthonormalizeColumn(PetscInt j, bool mgs, bool cgs2);
//   PetscReal OrthonormalizeColumn(PetscInt j, bool mgs, bool cgs2, const PetscParMatrix
//   &B,
//                                  PetscParVector &Bv);
