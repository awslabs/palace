...
ParOperator::ParOperator(std::unique_ptr<Operator> &&A,
                         ...
                         const mfem::ParFiniteElementSpace &test_fespace,
                         bool test_restrict, bool alloc)
  : ParOperator(std::move(A), nullptr, trial_fespace, test_fespace, test_restrict, alloc)
{
}
...
ParOperator::ParOperator(const Operator &A,
                         ...
                         const mfem::ParFiniteElementSpace &test_fespace,
                         bool test_restrict, bool alloc)
  : ParOperator(nullptr, &A, trial_fespace, test_fespace, test_restrict, alloc)
{
}
...
ComplexParOperator::ComplexParOperator(std::unique_ptr<Operator> &&dAr,
  ...
  : ComplexOperator(test_fespace.GetTrueVSize(), trial_fespace.GetTrueVSize()),
    ...
    RAPr(A->Real() ? std::make_unique<ParOperator>(*A->Real(), trial_fespace, test_fespace,
                                                   use_R, false)
                   : nullptr),
    RAPi(A->Imag() ? std::make_unique<ParOperator>(*A->Imag(), trial_fespace, test_fespace,
                                                   use_R, false)
                   : nullptr)
{
