...
ComplexVector::ComplexVector(ComplexVector &&y) noexcept : x(std::move(y.x)),
xr(x, 0, x.Size()/2), xi(x, x.Size()/2, x.Size()/2)
{
}
...
ComplexVector &ComplexVector::operator=(ComplexVector &&y) noexcept
{
  x = std::move(y.x);
  xr.MakeRef(x, 0, x.Size()/2);
  xr.MakeRef(x, x.Size()/2, x.Size()/2);
  return *this;
}
