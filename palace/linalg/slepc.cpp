...
static PetscErrorCode __form_NEP_function(NEP, PetscScalar, Mat, Mat,
                                          void *);
...
void SlepcNEPSolver::Customize()
{
  ...
  if (sinvert && region)
  {
    ...
    {
      PetscReal sr = PetscRealPart(sigma);
      double bound = 1.0e1 * std::abs(sr) / gamma; // what value to use? same bounds for real and imag?
      if (sr > 0.0)
      {
    ...
    {
      PetscReal si = PetscImaginaryPart(sigma);
      double bound = 1.0e1 * std::abs(si) / gamma; // what value to use? same bounds for real and imag?
      if (si > 0.0)
      {
  ...
  PalacePetscCall(NEPNLEIGSSetFullBasis(nep, PETSC_TRUE)); // This is needed otherwise we get a bunch of incorrect eigenvalues. Where to put this?
}
                                  ...
                                  EigenvalueSolver::ScaleType type)
{
  ...
  if (first)
  {
    ...
    //PalacePetscCall(
    //    MatShellSetOperation(T, MATOP_GET_DIAGONAL, (void (*)(void))__mat_diagonal_NEP_A));
    //PalacePetscCall(
  ...
  */
  ...
  if (first)
  {
    ...
    for (int i = 0; i < nsolve; i++)
    {
      // PalacePetscCall(KSPSetType(ksp[i],KSPBICG));
                                          ...
                                          ComplexVector &r) const
{
  ...
  auto A2 = space_op->GetExtraSystemMatrix<ComplexOperator>(l.imag(), Operator::DIAG_ZERO);
  auto A = space_op->GetSystemMatrix(std::complex<double>(1.0, 0.0), l, l * l,
                                     opK, opC, opM, A2.get());
  A->Mult(x, r);
...
PetscErrorCode __mat_apply_NEP_A(Mat A, Vec x, Vec y)
{
  ...
  //std::cerr << "In  __mat_apply_NEP_A ctx->opA->Width(): " << ctx->opA->Width() << "\n";
  //PetscCall(FromPetscVec(x, ctx->x1));
  //std::cerr << "In  __mat_apply_NEP_A after frompetscvec\n";
  //std::cerr << "In  __mat_apply_NEP_A ctx->x1.Size(): " << ctx->x1.Size() << " ctx->y1.Size(): " << ctx->y1.Size() << "\n";
  palace::ComplexVector t1(ctx->opA->Height()), t2(ctx->opA->Height());
  t1.UseDevice(true); t1 = 0.0;
  t2.UseDevice(true); t2 = 0.0;
  PetscCall(FromPetscVec(x, t1));
  ctx->opA->Mult(t1, t2); //try with t1 and t2 instead of x1 y1? NEED TO FIGURE OUT WHY IT FAILS WITH x1, y1?!?!!?!?!?!?
...
PetscErrorCode __form_NEP_function(NEP nep, PetscScalar lambda, Mat fun, Mat B, void *ctx)
{
  ...
  ctxF->opA2 = ctxF->space_op->GetExtraSystemMatrix<palace::ComplexOperator>(
      lambda.imag(), palace::Operator::DIAG_ZERO);
