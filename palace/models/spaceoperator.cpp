...
//{
  ...
  void SpaceOperator::AssemblePreconditioner(std::complex<double> a0, std::complex<double> a1,
  ...
  {
    Mpi::Print("Using AssemblePreconditioner with complex coefficients!\n");
    ...
    //AddPeriodicCoefficients(a0.real(), fr, fpi);//?
  ...
  void SpaceOperator::AssemblePreconditioner(double a0,double  a1, double a2, double a3,
                                      std::vector<std::unique_ptr<Operator>> &br_vec,
                            std::vector<std::unique_ptr<Operator>> &br_aux_vec )
  {
    constexpr bool skip_zeros = false, assemble_q_data = false;
...
template <typename OperType, typename ScalarType>
std::unique_ptr<OperType> SpaceOperator::GetPreconditionerMatrix(ScalarType a0, ScalarType a1,
...
void SpaceOperator::AddRealPeriodicCoefficients(double coeff, MaterialPropertyCoefficient &f)
{
...
void SpaceOperator::AddImagPeriodicCoefficients(double coeff, MaterialPropertyCoefficient &f)
{
  // Floquet periodicity contributions.
  if (mat_op.HasWaveVector())
  {
