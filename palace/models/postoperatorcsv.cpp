...
template <ProblemType solver_t>
void PostOperatorCSV<solver_t>::PrintFarFieldE()
{
  if (!farfield_E)
  {
    return;
  ...
  for (size_t i = 0; i < measurement_cache.farfield.thetaphis.size(); i++)
  {
    ...
    for (int i_dim = 0; i_dim < v_dim; i_dim++)
    {
      auto val = E_field[i_dim];
      farfield_E->table[format("rE{}_re", i_dim)] << val.real();
      if (HasComplexGridFunction<solver_t>())
      {
        farfield_E->table[format("rE{}_im", i_dim)] << val.imag();
      }
    }
  }
  farfield_E->WriteFullTableTrunc();
