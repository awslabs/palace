...
template <DielectricInterfaceType T>
using RestrictedInterfaceCoefficient =
    RestrictedCoefficient<DielectricInterfaceCoefficient<T>>;
    ...
    std::size_t i, const mfem::ParGridFunction &U, const MaterialOperator &mat_op) const
{
  switch (type)
  {
    case DielectricInterfaceType::MA:
      return std::make_unique<RestrictedInterfaceCoefficient<DielectricInterfaceType::MA>>(
          attr_lists[i], U, mat_op, ts, epsilon, sides[i]);
    case DielectricInterfaceType::MS:
      return std::make_unique<RestrictedInterfaceCoefficient<DielectricInterfaceType::MS>>(
          attr_lists[i], U, mat_op, ts, epsilon, sides[i]);
    case DielectricInterfaceType::SA:
      return std::make_unique<RestrictedInterfaceCoefficient<DielectricInterfaceType::SA>>(
          attr_lists[i], U, mat_op, ts, epsilon, sides[i]);
    case DielectricInterfaceType::DEFAULT:
      return std::make_unique<
          RestrictedInterfaceCoefficient<DielectricInterfaceType::DEFAULT>>(
          attr_lists[i], U, mat_op, ts, epsilon, sides[i]);
  }
