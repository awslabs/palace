...
public:
  ...
  // propagation constant kn, power-normalized Poynting P, and eigensolver error
  // estimates.
  template <ProblemType U = solver_t>
  auto MeasureAndPrintAll(int step, const ComplexVector &et, const ComplexVector &en,
                          std::complex<double> kn, std::complex<double> P, double omega,
                          double error_abs, double error_bkwd, int num_conv)
