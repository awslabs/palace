...
{
  return (kind == Kind::FLUX_Q || kind == Kind::ENERGY_E || kind == Kind::ENERGY_M) ? 1 : 3;
}
...
PointFieldEvaluator::PointFieldEvaluator(Kind kind, const Mesh &mesh,
  ...
  : location(MeshEntityType::Boundary), kind(kind),
    ...
    boundary_marker(bdr_attr_marker), boundary_fespace(&fespace), boundary_mat_op(&mat_op),
    boundary_lod(lod), boundary_scaling(scaling)
{
...
void PointFieldEvaluator::EnsureBoundaryEvaluator() const
{
  ...
  switch (boundary_type)
  {
    ...
    case BoundaryEvaluatorType::POYNTING:
      boundary_eval.reset(new SurfaceFunctional(
          kind, *boundary_mesh, boundary_marker, *boundary_nd_fespace, *boundary_rt_fespace,
          *boundary_mat_op, boundary_lod, boundary_scaling));
      break;
