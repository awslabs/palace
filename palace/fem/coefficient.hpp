...
public:
  ...
  EvalComplexBatch(mfem::ElementTransformation &T, const mfem::IntegrationPoint &ip)
  {
    ...
    // TODO: We are not failing fast here (this code is run after the simulation
    // is completed). It would be best to return an error at config-construction
    // time.
    auto IsIsotropic = [](const auto &data)
    {
      for (size_t i = 0; i < data.Size(); i++)
        for (size_t j = 0; j < data.Size(); j++)
          if (data(i, j) != (i == j ? data(0, 0) : 0.0))
            return false;
      return true;
    };

    MFEM_VERIFY(IsIsotropic(mat_op.GetInvImpedance(FET.Elem1->Attribute)),
                "FarField calculations require isotropic materials");
