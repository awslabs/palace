                                         ...
                                         double scaling)
{
  ...
  for (const auto &geom_indices : geom_elems)
  {
    ...
    // Element attributes for material lookup.
    ...
    // Mesh node gradients for on-the-fly geometry evaluation.
    ...
    // Source fields evaluated at the target nodal points.
    auto AddFieldInput =
    ...
    if (kind == Kind::ENERGY_E || kind == Kind::POYNTING || kind == Kind::MODE_SN)
    {
      AddFieldInput("u_1", 0, *nd_fespace);
    }
    if (kind == Kind::ENERGY_M || kind == Kind::POYNTING || kind == Kind::MODE_SN)
    {
      AddFieldInput((kind == Kind::POYNTING || kind == Kind::MODE_SN) ? "u_2" : "u_1", 1,
                    *rt_fespace);
    }

    // Scatter nodal values directly into the target grid function.
