...
void CreateSequentialPointRestriction(Ceed ceed, std::size_t num_elem, int nq,
                                      int num_comp, CeedSize l_size,
  ...
  : comm(mesh.GetComm())
{
      ...
      for (int s = 0; s < MaxSources; s++)
      {
        if (mask & (1u << s))
        {
          ...
          const bool at_points_group =
              use_at_points && geom == mfem::Geometry::TETRAHEDRON;
          PointConfigKey key;
  ...
  for (const auto &[key, group] : export_map)
  {
    ...
    if (group.at_points)
    {
      ...
      CreateSequentialPointRestriction(ceed, num_elem, nq, 3, points.Size(),
                                       &points_restr);
    ...
    if (group.at_points)
    {
      InitTetBasisForAtPoints(*mesh_fe, /*grad_only*/ true, mesh_fespace.GetVDim(),
                              ceed, &mesh_basis);
    }
    else
    {
    ...
    if (group.at_points)
    {
      ceed::AssembleCeedPointEvaluatorAtPoints(info, nullptr, 0, ceed, inputs,
                                               points_restr, points_vec, value_dim,
                                               out_restr, &op);
    }
    else
    {
