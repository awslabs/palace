...
// Key identifying one export point-evaluator group. Surface-functional requests carry
// the canonical reference-face/subface transformation key. Every key is extended with
// the exact mapped point-rule signature before it reaches the process-lifetime registry.
...
using SetupWord = std::uint64_t;

SetupWord EncodeSetupInt(int value)
{
  MFEM_VERIFY(value >= 0, "Negative integer in face-neighbor setup payload!");
  return static_cast<SetupWord>(value);
}

int DecodeSetupInt(SetupWord value)
{
  MFEM_VERIFY(value <= static_cast<SetupWord>(std::numeric_limits<int>::max()),
              "Invalid integer in face-neighbor setup payload!");
  ...
  : comm(mesh.GetComm())
{
  ...
  // Serialize setup in raw 64-bit words: element position, point count, source mask,
  // key length, key entries, then reference coordinates and weights. In particular, key
  // entries are not converted through double, so exact IEEE coefficient bits survive MPI
  // setup.
  ...
  std::vector<std::vector<SetupWord>> send_payload(num_nbr);
  int import_size = 0;
  for (int i = 0; i < num_nbr; i++)
  {
    ...
    {
      const auto &req = requests[r];
      MFEM_VERIFY(req.pts.size() <=
                          static_cast<std::size_t>(std::numeric_limits<int>::max()) &&
                      req.point_key.size() <=
                          static_cast<std::size_t>(std::numeric_limits<int>::max()),
                  "Face-neighbor setup payload exceeds int range!");
      const int nq = static_cast<int>(req.pts.size());
      payload.push_back(EncodeSetupInt(req.face_nbr_elem - elem_offsets[i]));
      payload.push_back(EncodeSetupInt(nq));
      payload.push_back(EncodeSetupInt(static_cast<int>(req.source_mask)));
      payload.push_back(EncodeSetupInt(static_cast<int>(req.point_key.size())));
      for (const auto value : req.point_key)
      {
        payload.push_back(EncodeSetupKey(value));
      }
      for (const auto &ip : req.pts)
      {
        payload.push_back(EncodeSetupDouble(ip.x));
        payload.push_back(EncodeSetupDouble(ip.y));
        payload.push_back(EncodeSetupDouble(ip.z));
        payload.push_back(EncodeSetupDouble(ip.weight));
      }
      for (int s = 0; s < MaxSources; s++)
      {
  ...
  // empty): payload sizes, then raw setup words.
  std::vector<int> send_len(num_nbr), recv_len(num_nbr);
  {
    std::vector<MPI_Request> mpi_reqs(2 * num_nbr);
    for (int i = 0; i < num_nbr; i++)
    {
      MFEM_VERIFY(send_payload[i].size() <=
                      static_cast<std::size_t>(std::numeric_limits<int>::max()),
                  "Face-neighbor setup payload exceeds MPI count range!");
  ...
  std::vector<std::vector<SetupWord>> recv_payload(num_nbr);
  {
    std::vector<MPI_Request> mpi_reqs;
    mpi_reqs.reserve(2 * num_nbr);
    for (int i = 0; i < num_nbr; i++)
    {
      MFEM_VERIFY(recv_len[i] >= 0, "Invalid face-neighbor setup payload length!");
      if (recv_len[i] > 0)
      {
        recv_payload[i].resize(recv_len[i]);
        MPI_Irecv(recv_payload[i].data(), recv_len[i], MPI_UINT64_T,
                  pmesh.GetFaceNbrRank(i), TAG_SETUP_PAYLOAD, comm,
                  &mpi_reqs.emplace_back());
      }
      if (send_len[i] > 0)
      {
        MPI_Isend(send_payload[i].data(), send_len[i], MPI_UINT64_T,
                  pmesh.GetFaceNbrRank(i), TAG_SETUP_PAYLOAD, comm,
                  &mpi_reqs.emplace_back());
      }
    }
  ...
  // Parse the received requests, assigning export offsets with the same layout rules as
  // the import offsets. Every group uses a fixed mapped integration rule and ordinary
  // libCEED basis/operator. Equal canonical keys batch across faces and senders; exact
  // point comparison remains a guard against an incomplete key.
  struct ExportGroup
  {
  ...
  for (int i = 0; i < num_nbr; i++)
  {
    ...
    auto ReadWord = [&]()
    {
      MFEM_VERIFY(k < payload.size(), "Truncated face-neighbor setup payload!");
      return payload[k++];
    };
    while (k < payload.size())
    {
      const int p = DecodeSetupInt(ReadWord());
      const int nq = DecodeSetupInt(ReadWord());
      const auto mask = static_cast<unsigned int>(DecodeSetupInt(ReadWord()));
      const int point_key_size = DecodeSetupInt(ReadWord());
      MFEM_VERIFY(
          point_key_size >= 0,
          "Invalid face-neighbor point-key size in received field exchange request!");
      std::vector<long long> point_key(point_key_size);
      for (int q = 0; q < point_key_size; q++)
      {
        point_key[q] = DecodeSetupKey(ReadWord());
      }
      MFEM_VERIFY(nq > 0 && p >= 0 && p < pmesh.send_face_nbr_elements.RowSize(i),
                  "Invalid face-neighbor request in setup payload!");
      const int elem = pmesh.send_face_nbr_elements.GetRow(i)[p];
      std::vector<mfem::IntegrationPoint> pts(nq);
      for (int q = 0; q < nq; q++)
      {
        // Read in wire order before Set3: function-argument evaluation order is not
        // specified, while each ReadWord advances the payload cursor.
        const double x = DecodeSetupDouble(ReadWord());
        const double y = DecodeSetupDouble(ReadWord());
        const double z = DecodeSetupDouble(ReadWord());
        const double weight = DecodeSetupDouble(ReadWord());
        pts[q].Set3(x, y, z);
        pts[q].weight = weight;
      }
      ...
      for (int s = 0; s < MaxSources; s++)
      {
        if (mask & (1u << s))
        {
          ...
          PointConfigKey key;
          key.reserve(5 + point_key.size() + 4 * pts.size());
          key.push_back(s);
          key.push_back(static_cast<long long>(geom));
          key.push_back(nq);
          key.push_back(point_key.empty() ? 0 : 1);
          key.push_back(static_cast<long long>(point_key.size()));
          key.insert(key.end(), point_key.begin(), point_key.end());
          // The point signature is required even with a canonical transformation key:
          // it is the immutable registry identity that prevents a reused ad-hoc key
          // from aliasing changed coordinates after an exchange rebuild.
  ...
  for (const auto &[key, group] : export_map)
  {
    ...
    MFEM_VERIFY(group.pts.size() == static_cast<std::size_t>(nq),
                "Invalid fixed export rule for face neighbor exchange!");
    ...
    // and the field, both evaluated at the fixed mapped rule.
  ...
  // exchange. Detach the borrowed arrays before releasing the exact-size source staging
  // vectors, which would otherwise add full device FE vectors to the evaluator.
  DetachFieldVectors();
