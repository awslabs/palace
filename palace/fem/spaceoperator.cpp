...
std::vector<double> SpaceOperator::GetErrorEstimates(const mfem::ParComplexGridFunction &E)
{
  ...
  // constexpr double normp = 1;
  // for (int i = 0; i < nelem; i++)
  // {
  //   const auto real_error = mfem::ComputeElementLpDistance(normp, i, flux_func.real(),
  //   smooth_flux_func.real()); const auto imag_error =
  //   mfem::ComputeElementLpDistance(normp, i, flux_func.imag(), smooth_flux_func.imag());
  ...
  // // std::transform(estimates.begin(), estimates.end(), estimates.begin(), [=](const
  // double& x){return x/norm;}); std::vector<double> estimates;
  // estimates.reserve(estimate_components.size());
  // std::transform(estimate_components.begin(), estimate_components.end(),
  // std::back_inserter(estimates),
  // [](const std::complex<double> &x){ return std::sqrt(x.real() * x.real() + x.imag() *
  // x.imag()); });

  // L2ZZErrorEstimator
  const int order = GetNDSpace().GetElementOrder(0);  // assumed no mixed p
  auto *const pmesh = GetNDSpace().GetParMesh();
  mfem::CurlCurlIntegrator flux_integrator;  // unit muinv
  mfem::RT_FECollection flux_fec(order - 1, 3);
  mfem::ParFiniteElementSpace flux_fes(pmesh, &flux_fec);

  mfem::ND_FECollection smooth_flux_fec(order, 3);
  mfem::ParFiniteElementSpace smooth_flux_fes(pmesh, &smooth_flux_fec);

  int normp = 1;
  mfem::Vector real_error;
  const auto mfem_total_real_error = mfem::L2ZZErrorEstimator(
      flux_integrator, E.real(), smooth_flux_fes, flux_fes, real_error, normp);
  mfem::Vector imag_error;
  const auto mfem_total_imag_error = mfem::L2ZZErrorEstimator(
      flux_integrator, E.imag(), smooth_flux_fes, flux_fes, imag_error, normp);

  auto magnitude = [](const auto &r, const auto &i) { return std::sqrt(r * r + i * i); };

  std::vector<double> estimates;
  estimates.reserve(real_error.Size());
  std::transform(real_error.begin(), real_error.end(), imag_error.begin(),
                 std::back_inserter(estimates), magnitude);

  // // Comparisons
  // const auto total_error = std::accumulate(estimate_components.begin(),
  // estimate_components.end(), std::complex<double>(0,0)); MFEM_ASSERT(real_error.Size() ==
  // estimate_components.size(), "Number of estimates must match: " << real_error.Size() <<
  // " "<< estimate_components.size()); MFEM_ASSERT(imag_error.Size() ==
  // estimate_components.size(), "Number of estimates must match: " << imag_error.Size() <<
  // " "<< estimate_components.size());

  // Mpi::Print("real mfem: {}, palace: {}\n", mfem_total_real_error, total_error.real());
  // Mpi::Print("imag mfem: {}, palace: {}\n", mfem_total_imag_error, total_error.imag());

  // std::cout.precision(3);
  // std::cout << std::scientific;
  // std::cout << "REAL\n";
  // std::cout << "e: ";
  // for (auto const &x : estimate_components)
  //   std::cout << x.real() << ", ";
  // std::cout << '\n';

  // std::cout << "m: ";
  // for (auto const& x : real_error)
  //   std::cout << x << ", ";
  // std::cout << '\n';
