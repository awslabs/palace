...
// elements. Keep every rule passed to InitBasisFromRule alive for the application
    ...
    : comm(fespace.GetComm())
  {
    ...
    for (int i = 0; i < npts; i++)
    {
      ...
      std::vector<std::pair<std::string, int>> field_sources;
      {
        ...
        ceed::InitBasisFromRule(*mesh_fe, ir, mesh_fespace.GetVDim(), ceed, &mesh_basis);
        ...
        ceed::InitBasisFromRule(*fe, ir, fespace.GetVDim(), ceed, &field_basis);
