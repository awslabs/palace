...
mfem::Array<int> GetElementEnrichmentDofs(const std::vector<ElementDof> &element_dofs,
                                    ...
                                    const mfem::ElementTransformation &transformation)
{
  if (h1_fe.GetDim() != 3 || h1_fe.GetGeomType() != mfem::Geometry::TETRAHEDRON ||
      ...
      transformation.OrderJ() != 0)
  {
    throw std::invalid_argument(
        "Singular element coupling requires an affine three-dimensional tetrahedral "
        "H1 finite element of positive order!");
  }
                              ...
                              double &jacobian_determinant)
{
  ...
  if (inverse_jacobian.Height() != 3 || inverse_jacobian.Width() != 3 ||
      !std::isfinite(jacobian_determinant) || !(jacobian_determinant > 0.0))
  {
    throw std::invalid_argument(
        "Singular field evaluation requires a positive square affine Jacobian!");
  }
    ...
    DuffyH1ReferenceTable *duffy_table)
{
  ...
  {
    throw std::invalid_argument(
        "Singular H1 coupling requires an affine three-dimensional tetrahedral finite "
    ...
    const AdaptiveAssemblyOptions &options)
{
  ...
  for (int element = 0; element < mesh->GetNE(); element++)
  {
    ...
    try
    {
      enrichment = AssembleElementEnrichmentMatrices(element_dofs, grad_lambda,
