                     ...
                     const EdgeSlopeOptions &options, FieldNorm &&field_norm_at_point)
{
  ...
  for (int element = 0; element < fespace.GetNE(); element++)
  {
    ...
    for (const auto &node : features.elements[element].nodes)
    {
      ...
      if (feature.id != node.vertex || feature.selected_segments.size() != 1 ||
          feature.selected_segments[0] >= features.selected_segments.size() ||
    ...
    const TriangleBarycentricPoint &lambda)
{
  ...
  if (element < 0 || element >= fespace.GetNE())
  {
    throw std::out_of_range(
        "Triangular singular H1 field evaluation element is out of range!");
  }
  ValidateInteriorTriangleBarycentricPoint(lambda);

  auto *transformation = fespace.GetElementTransformation(element);
  if (!transformation)
  {
    throw std::runtime_error("Triangular singular H1 field element has no transformation!");
  }
  double jacobian_determinant;
  const auto grad_lambda =
  ...
  const auto enrichment = EvaluateElementTriangleH1Enrichment(
      topology.elements[element], local_enrichment, lambda, grad_lambda);
  result.potential += enrichment.potential;
  for (int d = 0; d < 2; d++)
  {
  ...
  const auto integrate = [&](int order)
  {
    ...
    for (int node : singular_nodes)
    {
      result += IntegrateReferenceTriangleNodeDuffy(
          order, node, H1DuffyRadialPower, [&](const TriangleBarycentricPoint &lambda)
    ...
    const TriangleBarycentricPoint &lambda)
{
  ...
  {
    throw std::out_of_range(
        "Triangular singular ND field evaluation element is out of range!");
  }
  ValidateInteriorTriangleBarycentricPoint(lambda);
  ...
  if (!transformation)
  {
    throw std::runtime_error("Triangular singular ND element has no transformation!");
  }
  double jacobian_determinant;
  const auto grad_lambda =
      GetTriangleBarycentricGradients(*transformation, point, jacobian_determinant);
  ...
  transformation->SetIntPoint(&point);
  standard_field.GetCurl(*transformation, standard_curl);
  for (int d = 0; d < 2; d++)
  {
  ...
  const auto enrichment = EvaluateElementTriangleNDEnrichment(
      topology.elements[element], local_enrichment, lambda, grad_lambda);
  for (int d = 0; d < 2; d++)
  {
