                                   ...
                                   CeedOperator *op, CeedQFunctionContext *ctx_out)
{
  ...
  CeedBasis sum_basis;
  {
    ...
    if (num_qpts == 0)
    {
      // A one-sided face-neighbor contribution can consist entirely of fixed EVAL_NONE
      // data (canonical face geometry, imported physical field values, and identity
      // Piola data). Its common ordinary restriction still defines Q exactly; no
      // runtime coordinate restriction or host apply-time routing is needed.
      for (const auto &input : inputs)
      {
        if (input.ops & EvalMode::None)
        {
          CeedInt elem_size, num_comp;
          PalaceCeedCall(ceed, CeedElemRestrictionGetElementSize(input.restr, &elem_size));
          PalaceCeedCall(ceed, CeedElemRestrictionGetNumComponents(input.restr, &num_comp));
          MFEM_VERIFY(num_comp > 0 && elem_size % num_comp == 0,
                      "Invalid fixed point restriction for surface functional assembly!");
          num_qpts = elem_size / num_comp;
          break;
        }
      }
    }
